...
| Area | Change |
|------|--------|
| Site | Fixed `/demo/` GitHub Pages basePath (withBase) |
| Site | Loading / error status lines on explorer, evidence, demo, reputation |
| Site | Demo mode fixture for Studionet `demo-1` + local favorites |
| Site | `/submit/` Portal paste kit |
| API | `openapi.json` method map for Studio surface |
